...
# Inputs: images (W,H,3,D,D) with values between 0 and 1, file_name
...
def quicksave(true_images, colors, masks, schedule, file_name, quicksave_type):
    ...
    tmp = np.where(np.cumsum(schedule) < true_images.shape[0] - 1, np.cumsum(schedule), -1)  # NOTE: This only works for schedules with 0's and 1's
