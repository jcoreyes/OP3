...
class Stage1_CEM:
    ...
    def _random_shooting(self, actions, model, image_suffix):
        ...
        schedule = np.array([0, 0, 0, 0, 1])  # 4 refinement steps, 1 physics step
...
class Stage1_MPC:
    ...
    def run_plan(self, goal_image, env, action_selection_class, num_actions_to_take, true_data, filter_goal_image=None, remove_goal_latent=True):
        # Goal inference
        self.env = env
        goal_info = self.goal_inference(goal_image, filter_goal_image)

        # Note state acquisition is not required for this task
        ...
        for t in range(num_actions_to_take):
            ...
            # Note state update is not required for this task

            if remove_goal_latent:
...
def main(variant):
    ...
    for i, goal_idx in enumerate(goal_idxs):
        ...
        single_stats = mpc.run_plan(goal_image, env, cem_process, n_goal_obs, true_data,
                                    filter_goal_image=filter_goal_params, remove_goal_latent=variant["remove_goal_latents"])
    ...
    for s in structure_split:
        variant = dict(
            ...
            cem_args=dict(
                cem_steps = 2,
                num_samples = 100,
                time_horizon = 1,  # This is fixed for this task
            ),
            cost_args=dict(
            ...
            filter_goal_image = True,  # Usually want this true unless running an unfactorized model
            remove_goal_latents = True,  # Usually want this true unless running an unfactorized model
            structure=s,
            debug=args.debug,
        ...
        n_seeds = 1
        exp_prefix = 'iodine-mpc-stage1-k7-v2-unfactorized-fixed'   # Note: Change this for logging purposes!
