...
from op3.exp_variants.pickplace_model_params_info import params_to_info
...
class Stage3_CEM:
    ...
    def _get_latent_locs(self, model, hidden_state, file_name=None):
        ...
        if file_name is not None:
            ...
            plot_action_vals(self.env, ptu.get_numpy(normalized_weights), ptu.get_numpy(rand_actions),
                             "{}/{}_pick_locs".format(self.logging_dir, file_name), is_normalized=True)
            # plot_action_vals_2(self.env, ptu.get_numpy(interaction_attention), ptu.get_numpy(rand_actions),
            #                  "{}/{}_pick_locs".format(self.logging_dir, file_name), is_normalized=False)
...
# Input: env,  act_vals (B,K),  actions (B,A)
# Output: Image / heatmap of pick locations and corresponding act_vals
def plot_action_vals_2(env, act_vals, actions, file_name, is_normalized=False):
    ...
    norm_colors = colors.Normalize(vmin=0, vmax=1)
    ...
    # K separate 2d histograms
    for i in range(k):
        fig, ax = plt.subplots(1, figsize=(5, 3))
        rgba_colors = np.zeros((act_vals.shape[0], 4))  # (B,4)
        rgba_colors[:] = some_colors[i]  # (4)
        rgba_colors[:, -1] = normalized_act_vals[:, i]  # (B)
        ax.hist2d(actions[:, 0], actions[:, 1], bins=50, weights=normalized_act_vals[:, i], norm=norm_colors)
        make_error_boxes(ax, block_locs[:, 0], block_locs[:, 1], env.TOLERANCE)
        #         plt.axis('equal')
        axes = plt.gca()
        axes.set_xlim([-2.5, 2.5])
        axes.set_ylim([1, 4])
        plt.savefig("{}_{}_2dhistogram.png".format(file_name, i))
...
def main(variant):
    ...
    ######Start Model loading######
    if "op3_args" in variant.keys():  # Normal model
        ...
        from examples.mpc_v2.stage3.savp_wrapper_v2 import SAVP_MODEL
        m = SAVP_MODEL('/nfs/kun1/users/rishiv/Research/baseline/logs/pickplace_multienv_10k/ours_savp/',
                       'model-500000', 0.5, batch_size=20)
    ######End Model loading######
...
# CUDA_VISIBLE_DEVICES=2 python stage3_mpc.py -de 0 -s 0 -m [s64d64_v1_params,curriculum_aws_params,random_aws_params]
# Last used: CUDA_VISIBLE_DEVICES=1 python stage3_mpc.py -de 0 -s 1 -m savp curriculum_aws_params -mode ec2
