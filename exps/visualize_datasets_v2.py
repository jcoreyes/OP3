...
from op3.exp_variants.model_parameters import params_to_info
...
from examples.iodine.iodine_runner import load_dataset
...
def copy_to_save_file(dir_str):
    ...
    shutil.copy2(base + '/examples/mpc_v2/real_world/visualize_datasets_v2.py', dir_str + '/visualize_datasets_v2.py')
...
dataset_to_models = dict(
    cloth=[
        dict(saved_model_args = params_to_info["cloth_reg"],
             K=4,
             model_type="rprp"
             ),
    #     dict(saved_model_args = params_to_info["cloth_reg"],
    ...
    #          ),
    #     dict(saved_model_args = params_to_info["cloth_sequence"],
    #          K=4,
    #          model_type="next_step"
    #          ),
        dict(saved_model_args = params_to_info["cloth_static"],
             K=4,
             model_type="static"
             ),
    ],
...
# Example usage: CUDA_VISIBLE_DEVICES=2 python visualize_datasets_v2.py -da [cloth,poke,solid]
# Instructions:
...
#      - If you want to use the other options [analyze_mse,create_mse_graphs] , please be sure to update the .npy file paths!
#   4. Choose how many timesteps to run the above functions for. Set this is in variant[T] below
#   5. Execute bash command: CUDA_VISIBLE_DEVICES=? python visualize_datasets_v2.py -da [cloth,poke,solid]
if __name__ == "__main__":
    ...
    variant = dict(
        ...
        machine_type='g3.16xlarge'  # Ignore: Only for logging, NOT used for setting ec2 instances (do that in conf)
    )

    # Relevant options: 'here_no_doodad', 'local_docker', 'ec2'
    run_experiment(
        get_mse_from_dataset,  # Set this! get_mse_from_dataset, create_images_from_dataset, analyze_mse, create_mse_graphs
